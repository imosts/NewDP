...
void* safeMalloc(size_t size){
    printf("safeMalloc1:%d\n" ,size);
    heapNode *ptr = (heapNode *)malloc(sizeof(heapNode));
//    heapNode *ptr = (heapNode *)malloc(sizeof(heapNode) + size);
    if (ptr) {
        printf("safeMalloc2 :%lx\n" ,ptr);
//        ptr->addr = ptr + 1;
        ptr->addr = malloc(size);
        printf("safeMalloc3 :%lx\n" ,ptr->addr);
        ptr->head = ptr;
        printf("safeMalloc4 :%lx\n" ,ptr->head);
        ptr->next = NULL;
        printf("safeMalloc5 :%lx\n" ,getMultiPTR(ptr));
        return getMultiPTR(ptr);
    }else{
        printf("safeMalloc6 ERROR!\n");
        return NULL;
    }
}
...
void safeFree(void* ptr){
    ...
    printf("safeFree3 %lx\n",  nodeHeadP);
//    realloc(nodeHeadP, sizeof(heapNode));
...
void getPtr(void **nodeOrigin, void ** nodeOld, void *ptrNew){
    ...
    if (*nodeOrigin != NULL && !notMultiPTR(*nodeOrigin)) {
        ...
        if (abs((unsigned long)nodeP-(unsigned long)ptrNew) > 0x100000000) {
            printf("getPtr2 nodeP:%lx\n", nodeP);
            printf("getPtr2 ptrNew:%lx\n", ptrNew);
            *nodeOld = ptrNew;
        }else {
